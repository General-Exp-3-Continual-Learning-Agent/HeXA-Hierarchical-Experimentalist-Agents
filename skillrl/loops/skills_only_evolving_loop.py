"""HeXA *skills-only* ablation: offline-to-online evolving with success-only
trajectories and a mistakes-free skill bank.

Same Init × Update shape as HeXA (`evolving_loop.py`):
- Round 1: distill an initial bank from offline trajectories
- Round 2+: evolve the bank from (previous bank + new round's trajectories)
- batch size = `--seeds-per-round` (default 3, matching HeXA)

What's different vs HeXA:
- **Input filter**: only SUCCESSFUL trajectories are sent to the teacher.
  Failed trajectories are still collected during agent rounds (so accuracy
  numbers are comparable) but are NOT used to extract skills.
- **Output schema**: the teacher emits only level-specific skills. The
  resulting bank has zero Mistake entries for the level. The
  `--max-mistakes` flag is therefore unused here.

Works with both Claude (default) and Qwen actors / teachers — the actor is
selected by `--model` and the teacher by `--teacher-model`, identical to the
HeXA loop.

Usage:
    python -m skillrl.loops.skills_only_evolving_loop \
...
from skillrl.distillation.skills_only_distill import (
    run_skills_only_distillation,
    evolve_skill_bank_skills_only,
)
...
def run_skills_only_evolving_loop(
    level_name: str,
    ...
    max_mistakes_agent: int = 0,
    verbose: bool = False,
):
    """Skills-only HeXA ablation. See module docstring for the diff vs HeXA.

    Notes:
    - `max_mistakes_agent=0` by default since the bank has no mistakes — the
      retriever has nothing to inject under the "MISTAKES TO AVOID" block.
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # ── Resume detection ─────────────────────────────────────────
    progress_path = output_dir / "progress_skills_only_evolving.json"
    resume_from_round = 0
    ...
    # Auto-patch level-specific prompts where they exist. Currently catapult
    # has a factual-block variant of the skills-only prompts (matches the
    # discipline of teacher_prompts_catapult.py for the main HeXA pipeline).
    if level_name == "catapult":
        from skillrl.distillation import skills_only_prompts as _sop
        _sop.patch()
        print(
            "  [Prompts] Patched skills_only_distill with the catapult-specific "
            "factual-block variants (no strategy hints leaked to teacher)."
        )

    print(f"\n{'='*70}")
    print(f"  SKILLS-ONLY EVOLVING LOOP (HeXA ablation): {level_name}")
    print(
        f"  Rounds: {resume_from_round + 1}-{num_rounds}, "
        f"Seeds/round: {seeds_per_round}, Start seed: {start_seed}"
    )
    print(f"  Inputs: successful trajectories only (failures dropped before teacher)")
    print(f"  Output schema: skills only (no mistakes), max {max_skills} skills/level")
    print(f"  Actor model: {model_name}    Teacher model: {teacher_model}")
    print(f"{'='*70}")

    init_successes, init_failures = collect_trajectories(initial_traj_dir)
    print(
        f"\nInitial trajectories: {len(init_successes)} successes, "
        f"{len(init_failures)} failures (failures will be ignored)"
    )
    ...
    for round_num in range(resume_from_round + 1, num_rounds + 1):
        ...
        skill_bank_path = output_dir / f"skill_bank_skills_only_{round_num}.json"
        ...
        if round_num == 1:
            if skill_bank_path.exists():
                ...
                print(f"\n  Phase 1: Seeding skill bank from initial successes")
                bank = run_skills_only_distillation(
                    level_name=level_name,
                    initial_traj_dir=initial_traj_dir,
            ...
            if skill_bank_path.exists():
                ...
                prev_path = output_dir / f"skill_bank_skills_only_{round_num - 1}.json"
                if not prev_path.exists():
                    raise FileNotFoundError(f"Cannot resume: previous bank not found: {prev_path}")
                ...
                if not prev_round_dir.exists():
                    raise FileNotFoundError(
                        f"Cannot resume: previous round trajectories not found: {prev_round_dir}"
                    )

                bank = evolve_skill_bank_skills_only(
                    level_name=level_name,
                    prev_bank=prev_bank,
        ...
        round_accuracy = len(round_successes) / len(results) if results else 0.0
    ...
    # ── Final summary ───────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"  SKILLS-ONLY EVOLVING LOOP COMPLETE")
    print(f"{'='*70}")
    print(f"\n  Round-by-round results:")
    for s in round_stats:
        print(
            f"    Round {s['round']}: {s['successes']}/{s['successes'] + s['failures']} "
    ...
    if total_runs:
        print(f"\n  Overall: {total_successes}/{total_runs} = {total_successes/total_runs:.0%}")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "HeXA skills-only ablation: offline-to-online evolving, success-only "
            "trajectories, no mistakes in the skill bank. Works with Claude or Qwen."
        )
    )
    parser.add_argument("--level", type=str, required=True, help="Puzzle level to train on")
    parser.add_argument("--initial-traj-dir", type=str, required=True,
                        help="Directory with initial seed trajectories")
    parser.add_argument("--output-dir", type=str, default=None,
                        help="Output directory (default: skillrl/data/skills_only_evolving/{level})")
    parser.add_argument("--num-rounds", type=int, default=3)
    parser.add_argument("--seeds-per-round", type=int, default=3,
                        help="New seeds per round (default 3, matching HeXA)")
    ...
    parser.add_argument("--model", type=str, default="claude",
                        help="Agent model (claude / qwen2.5 / gpt-oss / mock / etc.)")
    parser.add_argument("--teacher-model", type=str, default=TEACHER_MODEL,
                        help="Teacher model for distillation and evolution")
    ...
    parser.add_argument("--max-mistakes-agent", type=int, default=0,
                        help="Max mistakes injected into agent prompt (default 0 — "
                             "the bank has no mistakes anyway)")
    ...
    if args.output_dir is None:
        args.output_dir = str(
            ...
            / "data" / "skills_only_evolving" / args.level
        )

    run_skills_only_evolving_loop(
